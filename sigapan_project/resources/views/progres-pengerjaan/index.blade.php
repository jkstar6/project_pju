                ...
                <table id="data-table" class="display stripe group" style="width:100%">
                    ...
                    <tbody>
                        ...
                        @foreach ($progresPengerjaan as $item)
                            ...
                            <tr data-id="{{ $item->id }}" data-aset_id="{{ $item->aset_pju_id }}"
                                data-kode="{{ $item->asetPju->kode_tiang ?? '-' }}" data-lokasi="{{ $lokasiText }}"
                                data-petugas="{{ $item->user->name ?? '-' }}" data-tahapan="{{ $tahapan }}"
                                data-keterangan="{{ $item->keterangan }}">

                                <td class="text-center">{{ $loop->iteration }}</td>
                                <td class="text-left"><strong
                                        class="text-primary-500">{{ $item->asetPju->kode_tiang ?? '-' }}</strong></td>

                                {{-- Lokasi --}}
                                <td class="text-left">
                                    <div class="flex flex-col">
                                        <span
                                            class="font-medium text-gray-700 dark:text-gray-200">{{ $lokasiText }}</span>
                                        @if ($lat && $long)
                                ...
                                <td class="text-center">
                                    <div class="w-full max-w-[220px] mx-auto">
                                        <div class="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700 overflow-hidden">
                                            <div class="progress-bar bg-primary-500 h-2.5 rounded-full"
                                                style="width: {{ $persen }}%"></div>
                                        </div>
                                        <span
                                            class="progress-text text-xs text-gray-600 dark:text-gray-400">{{ $persen }}%</span>
                                    </div>
                                </td>
                                ...
                                <td class="text-center">
                                    <div class="flex items-center gap-[12px] justify-center">
                                        <button type="button" class="btn-icon btn-edit text-blue-600 custom-tooltip"
                                            data-text="Edit">
    ...
    <div id="modalCreate" class="modal-overlay fixed inset-0 z-[999] items-center justify-center bg-black/50 p-4">
        <div class="w-full max-w-2xl rounded-md bg-white dark:bg-[#0c1427] p-5">
            <div class="flex items-center justify-between mb-4">
                <h5 class="mb-0 font-semibold text-gray-800 dark:text-gray-100">
            ...
            <form action="{{ route('progres-pengerjaan.store') }}" method="POST" class="grid grid-cols-1 gap-4">
                ...
                <div>
                    ...
                    @if ($listAset->isEmpty())
                        <p class="text-xs text-amber-500 mt-1">
                            Semua aset sudah dalam pengerjaan.
                        </p>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <select name="tahapan"
                        ...
                        <option value="Pemasangan Tiang dan Armatur">
                            Pemasangan Tiang dan Armatur (60%)
                        </option>
                        <option value="Pemasangan Jaringan">
                            Pemasangan Jaringan (80%)
                        </option>
            ...
            <form id="formEdit" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="edit_kode"
                        class="w-full mt-1 border rounded-md px-3 py-2 bg-gray-100 dark:bg-[#15203c] dark:border-[#15203c]"
                        readonly>
                </div>
